...
const passwordWithLengthLowerThanRequiredMessage =
  "The password should have length of 8";
...
describe("Password validator form", () => {
  ...
  it("Should clear previous validation error when current password is valid", () => {
    ...
    expect(
      screen.queryByText(passwordWithoutNumbersMessage)
  ...
  it("Should clear previous validation error of not required length when current password is valid", () => {
    ...
    expect(
      screen.queryByText(passwordWithLengthLowerThanRequiredMessage)
    ).not.toBeInTheDocument();
  });
